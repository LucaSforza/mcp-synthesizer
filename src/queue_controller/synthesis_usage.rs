...
pub fn write_usage_to_test_run(
    ...
    redis::pipe()
        .cmd("HSET").arg(&key).arg("totalInputTokens").arg(totals.input_tokens).ignore()
        .cmd("HSET").arg(&key).arg("totalOutputTokens").arg(totals.output_tokens).ignore()
        .cmd("HSET").arg(&key).arg("cost_of_synthesis_USD").arg(totals.cost_usd).ignore()
        .query::<()>(conn)
...
mod tests {
    ...
    fn test_parse_summary_top_level() {
        ...
        assert_eq!((t.input_tokens, t.output_tokens, t.cache_read_input_tokens), (49800, 3939, 568458));
    ...
    fn test_parse_file_last_line() {
        ...
        writeln!(tmp.as_file(), r#"{{"inputTokens":999,"outputTokens":88,"cacheReadInputTokens":50,"costUSD":1.23}}"#).unwrap();
        tmp.flush().unwrap();
        let t = parse_output_file(tmp.path()).unwrap();
        assert_eq!((t.input_tokens, t.output_tokens, t.cache_read_input_tokens), (999, 88, 50));
    ...
    fn seed(conn: &mut redis::Connection, name: &str) -> (i64, i64) {
        ...
        redis::cmd("SET").arg(format!("project:name:{name}")).arg(pid).query::<()>(conn).unwrap();
        redis::cmd("HSET").arg(format!("project:{pid}")).arg("name").arg(name)
            .arg("number_invariants").arg(5).arg("created_at").arg("now").query::<()>(conn).unwrap();
        let trid: i64 = redis::cmd("INCR").arg("test_run:ids").query(conn).unwrap();
        redis::cmd("HSET").arg(format!("test_run:{trid}")).arg("project_id").arg(pid)
            .arg("compilation_passed").arg(0).arg("compilation_not_passed").arg(0)
            .arg("created_at").arg("now").query::<()>(conn).unwrap();
        redis::cmd("SADD").arg(format!("test_run:by_project:{pid}")).arg(trid).query::<()>(conn).unwrap();
        (pid, trid)
    }

    #[test]
    fn test_write_usage() {
        let Some(mut conn) = test_redis_conn() else { return };
        let (_, trid) = seed(&mut conn, "p");
        let totals = UsageTotals { input_tokens: 100, output_tokens: 50, cache_read_input_tokens: 30, cost_usd: 0.25 };
        write_usage_to_test_run(&mut conn, "p", &totals).unwrap();
        let fields: std::collections::HashMap<String, String> =
            redis::cmd("HGETALL").arg(format!("test_run:{trid}")).query(&mut conn).unwrap();
